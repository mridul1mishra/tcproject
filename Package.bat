...
"C:\Program Files (x86)\MSBuild\14.0\Bin\MsBuild.exe" /m /t:Rebuild "test_app.sln" /p:Configuration=%1;ContentAuthoringOutputDir=%~dp0_Deployment\Output\CAS;ContentDeliveryOutputDir=%~dp0_Deployment\Output\CDS
IF ERRORLEVEL 1 GOTO errorHandling

.\packages\nuget\nuget.exe pack ".\test_app\test_app.nuspec" -Version %2 -Verbosity detailed -OutputDirectory .\_Deployment\Packages
IF ERRORLEVEL 1 GOTO errorHandling
